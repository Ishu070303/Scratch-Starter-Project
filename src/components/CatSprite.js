...
import { updateSprite } from "../redux/slices/spritesSlice";

export default function CatSprite({ id }) {
  const dispatch = useDispatch();
  const sprite = useSelector(
    (state) =>
      state.sprites.sprites.find((sprite) => sprite.id === id) || {
        x: 0,
        y: 0,
        rotation: 0,
        speech: "",
  ...
  const spriteStyle = {
    ...
    visibility: sprite.hidden ? "hidden" : "visible",
  };
  ...
  const [, drop] = useDrop({
    ...
    drop: (item, monitor) => {
      ...
      const newX = Math.round(sprite.x + delta.x);
      const newY = Math.round(sprite.y + delta.y);
      dispatch(updateSprite({ id: item.id, changes: { x: newX, y: newY } }));
    },
  });
  return (
    <div ref={drag} className="cursor-grab" style={{ position: "relative", display: "inline-block" }}>
      <div ref={drop} className="cursor-grabbing">
        <svg
